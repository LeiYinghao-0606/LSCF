...
def training(model, args, dataset, device, logger):
    ...
    # 粗略估算参数占用显存/内存（仅参数，不含梯度、优化器状态、激活等）
    param_mem_mb_fp32 = total_params * 4 / (1024 ** 2)

    msg = (f"\t Model Params | total: {total_params:,} | trainable: {trainable_params:,} "
           f"| param_mem(fp32): {param_mem_mb_fp32:.2f} MB")
    print(msg)
    logger.info(msg)
    # =============================
